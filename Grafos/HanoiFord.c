...
void inicializa(int *distancia,int *caminho){
    for(int i = 0; i < nVertices; i++){
        distancia[i] = INF;
        caminho[i] = -1;
    }
...
void BellmanFord(Grafo *graph, int origem, int destino){
    int distancia[nVertices]; //armazena as menores distancias de cada vértice
    int caminho[nVertices];//armazena os vértices do caminho mais curto

    inicializa(distancia,caminho);

    distancia[origem] = 0; //a distancia da origem até ela mesma é sempre zero

    for(int i = 1; i < nVertices -1 ;i++){
        for(int j = 0; j < nVertices; j++){
            for(int v = 0; v < nVertices; v++){
                if(graph->matriz[j][v] == 1 && distancia[j] != INF && distancia[j] + peso < distancia[v]){
                    distancia[v] = distancia[j] + peso; //atualiza amenor distancia
                    caminho[v] = j; //adiciona o vértice ao caminho  
                }
            }
        }
    }
    ...
    for(int i = 0; i < nVertices; i++){
        for(int j = 0; j < nVertices; j++){
            if(graph->matriz[i][j] == 1 && distancia[i] != INF && distancia[i] + peso < distancia[j]){
                   printf("O grafo possui ciclos de peso negativo");
            }
        }
    }
...
int menu(){
    ...
    printf("4 - Jogar.\n");
    printf("5 - Calcular siatância com Bellman - Ford");
    printf("0 - Sair.\n");
...
int main(){
    ...
    while ((opc = menu())!=0)
    {
        ...
        case 4:
            ...
            printf("Deseja mover os discos para qual torre? (1 - Torre2 2 - Torre3)");
            scanf("%d",&torre);
        ...
        case 5:
            ...
            if(origem >= 0 && origem <= 80){
                ...
                if(destino >= origem && destino >= 0 && destino <= 80){
                    BellmanFord(graph,origem,destino);
                }else{
